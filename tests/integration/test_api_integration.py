...
class TestAuthenticationIntegration:
    ...
    def test_session_expiry_workflow(self):
        ...
        with patch('app.auth.get_settings') as mock_settings:
            ...
            mock_settings.return_value.algorithm = "HS256"
            mock_settings.return_value.access_token_expire_minutes = 1  # Very short expiry
...
class TestAPIWorkflowIntegration:
    ...
    def test_infrastructure_test_workflow(self):
        ...
        if status_data["available_tests"]:
            ...
            response = self.client.post(f"/api/tests/{test_id}", headers=self.auth_headers)
    ...
    def test_custom_ai_test_workflow(self):
        ...
        with patch('app.tests.openai_test.OpenAITest') as MockOpenAITest:
            ...
            response = self.client.post("/api/tests/openai/custom", 
    ...
    def test_error_handling_workflow(self):
        ...
        # Test 404 handling
        response = self.client.get("/api/nonexistent/endpoint", headers=self.auth_headers)
        assert response.status_code == 404
        
        # Test invalid test ID
        response = self.client.post("/api/tests/nonexistent_test", headers=self.auth_headers)
