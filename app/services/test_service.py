...
class TestService(BaseService):
    ...
    async def _register_all_tests(self) -> None:
        ...
        test_classes = [
            PostgresTest, CassandraTest, BlobStorageTest, S3Test, MinioTest,
            OpenAITest, LlamaTest, EmbeddingTest, DocumentIntelligenceTest,
            SSLTest, PVCTest
        ]
        ...
        for test_class in test_classes:
            try:
                ...
                self.logger.error(f"Failed to register test {test_class.__name__}: {str(e)}")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        configured_count = sum(1 for test in self.test_suite.tests.values() if test.is_configured())
    ...
    async def get_test_info(self, test_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "depends_on": test.depends_on,
            "timeout_seconds": test.timeout_seconds,
            "configuration_help": test.get_configuration_help() if not test.is_configured() else None,
    ...
    async def execute_test(
        ...
        self.log_operation("execute_test", {
            "test_id": test_id,
            "user_id": user_id,
        ...
        try:
            ...
            self.log_operation("test_completed", {
                "test_id": test_id,
    ...
    async def execute_test_async(
        ...
        task = asyncio.create_task(
            self.execute_test(test_id, user_id, custom_config)
        )
        
        task_id = f"{test_id}_{datetime.utcnow().isoformat()}"
    ...
    async def execute_test_suite(
        ...
        self.log_operation("execute_test_suite", {
        ...
        try:
            ...
            self.log_operation("test_suite_completed", {
