...
class InputSanitizer:
    ...
    def sanitize_prompt_input(prompt: str, max_length: int = 4000) -> str:
        ...
        if len(sanitized.strip()) < 3:
            raise HTTPException(status_code=400, detail="Prompt too short (minimum 3 characters)")
    ...
    @staticmethod
    async def validate_file_upload(file: UploadFile, max_size_mb: int = 10) -> Dict[str, Any]:
        """
        ...
        if not file_valid and file_ext != 'txt':  # Text files can be empty
            raise HTTPException(
                status_code=400, 
                detail="File content doesn't match expected file type"
            )
        ...
        for text in texts:
            if text:  # Skip empty strings
                ...
                if sanitized and len(sanitized) >= 3:  # Minimum viable text
                    sanitized_texts.append(sanitized[:1000])  # Limit individual text length
    ...
    def sanitize_json_input(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if isinstance(value, str):
                ...
                sanitized[safe_key] = [
                    InputSanitizer.sanitize_text_input(str(item)) if isinstance(item, str) else item
