...
class FileUploadHandler:
    ...
    async def process_upload(
        cls, file: UploadFile, allowed_types: set, max_size_mb: int = 25
    ) -> ProcessedFile:
        """
        ...
        if extension in cls.TEXT_EXTENSIONS:
            processed_content, file_type = await cls.process_text_file(
                content, extension
            )
            encoding = "utf-8"
        elif extension == "pdf":
            ...
            processed_content, file_type = await cls.process_image_file(
                content, extension
            )
            encoding = "base64"
        else:
    ...
    @classmethod
    async def process_ai_model_upload(
        cls, file: Optional[UploadFile]
        ...
        return await cls.process_upload(
            file=file, allowed_types=cls.AI_MODEL_TYPES, max_size_mb=25
        )
    ...
    async def process_document_intel_upload(cls, file: UploadFile) -> ProcessedFile:
        ...
        """
        return await cls.process_upload(
            file=file, allowed_types=cls.DOCUMENT_INTEL_TYPES, max_size_mb=25
        )

    @classmethod
    async def process_embedding_upload(
        cls, file: Optional[UploadFile]
    ) -> Optional[ProcessedFile]:
        """
        ...
        return await cls.process_upload(
            file=file, allowed_types=cls.EMBEDDING_TYPES, max_size_mb=25
        )
