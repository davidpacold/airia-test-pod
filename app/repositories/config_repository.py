...
class ConfigRepository(InMemoryRepository):
    ...
    async def list_configurations(
        ...
        self.log_operation("list_configurations", {
            "prefix": prefix,
    ...
    async def get_config_history(
        ...
        self.log_operation("get_config_history", {
            "key": key,
            "user_id": user_id,
            "limit": limit,
    ...
    async def backup_configurations(self) -> Dict[str, Any]:
        ...
        self.log_operation("backup_configurations", {
            "configuration_count": len(configurations)
        })
        ...
        for key, config_item in backup_configurations.items():
            if key in current_configurations:
                if overwrite_existing:
                    ...
                    await self._record_config_change(key, old_value, new_value, user_id, "restore")
                    
                    current_configurations[key] = config_item.copy()
                    current_configurations[key]["updated_by"] = user_id
                    current_configurations[key]["updated_at"] = datetime.utcnow().isoformat()
                    overwritten_count += 1
                else:
                    skipped_count += 1
                    continue
                ...
                await self._record_config_change(key, None, config_item.get("value"), user_id, "restore")
    ...
    async def get_config_statistics(self) -> Dict[str, Any]:
        ...
        self.log_operation("get_config_statistics", {
            "total_configurations": len(configurations),
            "total_changes": len(history)
        })
    ...
    async def cleanup_old_history(self, days_to_keep: int = 90) -> int:
        ...
        self.log_operation("cleanup_old_history", {
