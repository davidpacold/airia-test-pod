...
class TestRepository(InMemoryRepository):
    ...
    async def store_test_result(
        ...
        self.log_operation("store_test_result", {
            "result_id": result_id,
            "test_id": test_id,
    ...
    async def get_test_history(
        ...
        self.log_operation("get_test_history", {
            "test_id": test_id,
            "user_id": user_id,
            "limit": limit,
    ...
    async def get_test_statistics(
        ...
        durations = [r.get("duration_seconds") for r in filtered_results if r.get("duration_seconds")]
        ...
        for result in filtered_results:
            ...
            if tid not in test_breakdown:
                test_breakdown[tid] = {"total": 0, "passed": 0, "failed": 0, "skipped": 0}
            test_breakdown[tid]["total"] += 1
            test_breakdown[tid][result.get("status", "unknown")] += 1
            
        stats = {
            "total_executions": total_executions,
            "success_rate": round(passed_count / total_executions * 100, 2) if total_executions > 0 else 0.0,
        ...
        self.log_operation("get_test_statistics", {
            "test_id": test_id,
            "days": days,
            "total_executions": total_executions,
    ...
    async def store_test_configuration(
        ...
        self.log_operation("store_test_configuration", {
            "test_id": test_id,
            "user_id": user_id,
    ...
    async def delete_test_configuration(self, test_id: str, config_id: str) -> bool:
        ...
        for i, config in enumerate(test_configs):
            if config.get("config_id") == config_id:
                ...
                self.log_operation("delete_test_configuration", {
                    "test_id": test_id,
                    "config_id": config_id
                })
                return True
    ...
    async def cleanup_old_results(self, days_to_keep: int = 90) -> int:
        ...
        self.log_operation("cleanup_old_results", {
