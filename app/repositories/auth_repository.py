...
class AuthRepository(InMemoryRepository):
    ...
    async def update_password(self, user_id: str, password_hash: str, salt: str) -> bool:
        ...
        if user_id in users:
            ...
            users[user_id]["password_updated_at"] = datetime.utcnow().isoformat()
            await self.store_data("users", users)
    ...
    async def update_user_role(self, user_id: str, role: str, updated_by: str) -> bool:
        ...
        if user_id in users:
            ...
            users[user_id]["role_updated_at"] = datetime.utcnow().isoformat()
            users[user_id]["role_updated_by"] = updated_by
            await self.store_data("users", users)
            
            self.log_operation("update_user_role", {
                "user_id": user_id,
                "old_role": old_role,
                "new_role": role,
    ...
    async def deactivate_user(self, user_id: str, deactivated_by: str) -> bool:
        ...
        if user_id in users and users[user_id].get("is_active", True):
            ...
            users[user_id]["deactivated_at"] = datetime.utcnow().isoformat()
    ...
    async def record_user_activity(
        ...
        self.log_operation("record_user_activity", {
            "user_id": user_id,
            "activity_type": activity_type
        })
    ...
    async def _clear_user_sessions(self, user_id: str) -> None:
        ...
        self.log_operation("clear_user_sessions", {
            "user_id": user_id,
    ...
    async def cleanup_old_activity(self, days_to_keep: int = 90) -> int:
        ...
        self.log_operation("cleanup_old_activity", {
