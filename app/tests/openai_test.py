...
class OpenAITest(BaseTest):
    ...
    def get_configuration_help(self) -> str:
        return ("AI model API configuration required. Supports Azure OpenAI, OpenAI, Llama, and compatible APIs. "
    ...
    def run_test(self) -> TestResult:
        ...
        try:
            ...
            if use_azure:
                result.add_log("INFO", f"Using Azure OpenAI configuration: {self.azure_endpoint}")
                result.add_log("INFO", f"Model deployment: {self.azure_deployment_name}")
                client = AzureOpenAI(
                ...
                result.add_log("INFO", f"Using OpenAI-compatible API: {self.openai_base_url}")
                ...
                if "llama" in self.openai_model_name.lower():
                    result.add_log("INFO", "Detected Llama model - testing compatibility")
                client = OpenAI(
            ...
            if self.test_completion and completion_model:
                completion_result = self._test_completion(client, completion_model, use_azure)
                result.add_sub_test("Text Completion", completion_result)
                if not completion_result["success"]:
                    all_passed = False
            else:
                result.add_log("INFO", "Skipping completion test - not configured or disabled")
            
            # Test 3: Embedding API
            if self.test_embedding and embedding_model:
                embedding_result = self._test_embedding(client, embedding_model, use_azure)
            ...
            if all_passed:
                ...
                failed_tests = [name for name, test_result in result.sub_tests.items() 
            ...
            if not use_azure:
                try:
                    ...
                    return {
                        ...
                        "models": model_list[:10],  # Limit to first 10 models for display
    ...
    def _basic_connection_test(self, client, use_azure: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            test_model = self.azure_deployment_name if use_azure else self.openai_model_name
    ...
    def _test_embedding(self, client, model: str, use_azure: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            has_values = any(abs(val) > 0.001 for val in embedding_vector[:10])  # Check first 10 values
