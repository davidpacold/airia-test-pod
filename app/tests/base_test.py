...
class BaseTest(ABC):
    ...
    def execute(self) -> TestResult:
        ...
        result.add_log("INFO", f"Completed {self.test_name} with status {result.status.value}")
        self.logger.info(f"Completed {self.test_name} with status {result.status.value}")
        return result
