...
class CassandraTest(BaseTest):
    ...
    def is_configured(self) -> bool:
        ...
        return bool(self.settings.cassandra_hosts and self.settings.cassandra_hosts != "")
        
    def get_configuration_help(self) -> str:
        """Return configuration help"""
        return ("Configure Cassandra connection using environment variables: "
    ...
    def get_cluster_config(self) -> Dict[str, Any]:
        ...
        config = {
            'contact_points': [h.strip() for h in self.settings.cassandra_hosts.split(',')],
        ...
        if self.settings.cassandra_datacenter:
            config['load_balancing_policy'] = DCAwareRoundRobinPolicy(local_dc=self.settings.cassandra_datacenter)
    ...
    def _test_cluster_health(self, cluster) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["details"]["up_nodes"] == 0:
                ...
                result["success"] = True
                result["message"] = f"Cluster '{result['cluster_name']}' has {result['details']['up_nodes']} nodes up"
    ...
    def _test_list_keyspaces(self, session) -> Dict[str, Any]:
        ...
        try:
            ...
            result["details"] = {
                "user_keyspace_count": len(keyspaces)
            }
            ...
            if self.settings.cassandra_keyspace:
                if self.settings.cassandra_keyspace in keyspaces:
                    result["message"] = f"Found configured keyspace: {self.settings.cassandra_keyspace}"
                    result["details"]["configured_keyspace_exists"] = True
                else:
                    result["message"] = f"Configured keyspace '{self.settings.cassandra_keyspace}' not found"
    ...
    def _test_replication_settings(self, session) -> Dict[str, Any]:
        ...
        try:
            ...
            result["message"] = f"Retrieved replication settings for {len(replication_info)} keyspaces"
            
            # Check specific keyspace if configured
            if self.settings.cassandra_keyspace and self.settings.cassandra_keyspace in replication_info:
                result["details"]["configured_keyspace_replication"] = replication_info[self.settings.cassandra_keyspace]
