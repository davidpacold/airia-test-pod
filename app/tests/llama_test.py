...
class LlamaTest(BaseTest):
    ...
    def __init__(self):
        ...
        self.llama_api_key = os.getenv("LLAMA_API_KEY", "not-required")  # Many self-hosted don't need keys
    ...
    def is_configured(self) -> bool:
        ...
        openai_llama_configured = (
            ...
            "llama" in self.openai_model_name.lower()
        )
        
        return llama_configured or openai_llama_configured
        
    def get_configuration_help(self) -> str:
        return ("Llama model API configuration required. "
    ...
    def run_test(self) -> TestResult:
        ...
        try:
            ...
            if use_llama_config:
                result.add_log("INFO", f"Using Llama API configuration: {self.llama_base_url}")
                ...
                result.add_log("INFO", "Detected Llama model via OpenAI-compatible interface")
                client = OpenAI(
            ...
            completion_result = self._test_completion(client, model_name, max_tokens, temperature)
            result.add_sub_test("Text Generation", completion_result)
            if not completion_result["success"]:
                all_passed = False
            
            # Test 3: Llama-specific prompt test
            llama_prompt_result = self._test_llama_prompt(client, model_name, max_tokens, temperature)
    ...
    def _test_completion(self, client: OpenAI, model_name: str, max_tokens: int, temperature: float) -> Dict[str, Any]:
    ...
    def _test_llama_prompt(self, client: OpenAI, model_name: str, max_tokens: int, temperature: float) -> Dict[str, Any]:
...
What are the key capabilities of Llama models? [/INST]"""
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": "What are the key capabilities of Llama models? Please answer concisely."}
                ],
                max_tokens=max_tokens,
                temperature=temperature
            )
